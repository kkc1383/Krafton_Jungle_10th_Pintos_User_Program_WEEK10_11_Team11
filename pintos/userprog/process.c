...
static struct lock child_all_list_lock;
...
static void __do_fork(struct fork_aux *aux) {
  ...
  struct thread *current = thread_current();
  struct child_process *cp = local_aux.cp;
  cp->tid = current->tid;
  current->self_cp = cp;
